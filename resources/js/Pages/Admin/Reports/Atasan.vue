...
const props = defineProps<{ periods: Period[]; targets: Target[]; selectedPeriod: Period | null; selectedTarget: { id: number; name: string; nik?: string; avatar_url?: string | null; departments?: string | null } | null; targetPosition: { id: number; name: string } | null; template: { id: number; target_category: string; description?: string | null } | null; rows: Row[]; evaluatorCount: number; filters: { period: number | null; target: number | null } }>();
const filters = { period: props.filters.period ?? props.selectedPeriod?.id ?? '', target: props.filters.target ?? '' };
const targetSearch = ref('');
const showSuggestions = ref(false);
const showProfileZoom = ref(false);
...
<template>
    ...
    <AdminLayout title="Laporan Evaluasi per Atasan">
        <div class="space-y-4">
            ...
            <div v-if="selectedTarget" class="report-content rounded-xl bg-white p-5"><div class="flex items-center gap-3"><button v-if="selectedTarget.avatar_url" type="button" class="no-print rounded-full" aria-label="Perbesar foto profil" @click="showProfileZoom = true"><img :src="selectedTarget.avatar_url" :alt="selectedTarget.name" class="h-14 w-14 rounded-full object-cover" /></button><span v-else class="flex h-14 w-14 items-center justify-center rounded-full bg-blue-100 text-xl font-bold text-blue-700">{{ selectedTarget.name.slice(0, 1) }}</span><div><h2 class="text-lg font-bold">{{ selectedTarget.name }}<span v-if="targetPosition"> - {{ targetPosition.name }}</span></h2><p class="text-sm text-slate-500">{{ selectedPeriod?.month }}/{{ selectedPeriod?.year }} · {{ selectedTarget.nik ?? '-' }} · Departemen: {{ selectedTarget.departments ?? '-' }} · Template: {{ template?.target_category?.toUpperCase() ?? '-' }} · <Link :href="route('admin.reports.evaluations.atasan.evaluators', { period: selectedPeriod?.id, target: selectedTarget.id })" class="no-print font-semibold text-blue-700 underline">Jumlah Penilai: {{ evaluatorCount }} orang</Link><span class="hidden print:inline">Jumlah Penilai: {{ evaluatorCount }} orang</span></p></div></div></div>
          ...
          <div v-if="showProfileZoom" class="no-print fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 p-4" @click.self="showProfileZoom = false">
              <div class="relative"><button type="button" class="absolute -right-3 -top-3 flex h-9 w-9 items-center justify-center rounded-full bg-white text-xl" aria-label="Tutup foto profil" @click="showProfileZoom = false">×</button><img :src="selectedTarget?.avatar_url ?? ''" :alt="selectedTarget?.name ?? 'Foto profil'" class="max-h-[80vh] max-w-[90vw] rounded-xl object-contain" /></div>
          </div>
      </AdminLayout>
 </template>
